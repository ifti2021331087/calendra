...
export async function createEvent(
    unsafeData: z.infer<typeof eventFormSchema>): Promise<{error:boolean} | undefined> {
    try {
...
export async function deleteEvent(id:string):Promise<void> {
    try{
        ...
        if(rowCount==0){
            throw new Error("Event not found or user not authenticated to delete this event!")
        }
    }
    catch(error:any){
        throw new Error(`Failed to delete the event: ${error.message || error}`)
    }
    finally{
    }
    revalidatePath("/events");
